...
import com.sgfootcal.android.footcal.Adapter.GoalScorerAdapter;
...
public class TopScorerFragment extends Fragment implements SearchView.OnQueryTextListener{
    ...
    public boolean onQueryTextChange(String newText) {
        if (isOnline()) {
            searchAllFootballerByLeagueId(newText, 1);
        }
        return false;
    }
    @Override
    ...
    public void getTheTopScorer(int league_Id){

        footballerDaoInterface.allFootballerByLeagueIdTopScorer(league_Id).enqueue(new Callback<FootballerSample>() {
            @Override
            public void onResponse(Call<FootballerSample> call, Response<FootballerSample> response) {
                if (response.isSuccessful()){
                    ...
                    goalScorerAdapter = new GoalScorerAdapter(getContext(),footballerList);
    ...
    public void searchAllFootballerByLeagueId(final String searchKey,int League_Id){

        footballerDaoInterface.searchAllFootballerByLeagueId(searchKey,League_Id).enqueue(new Callback<FootballerSample>() {
            @Override
            public void onResponse(Call<FootballerSample> call, Response<FootballerSample> response) {
                if (response.isSuccessful()){
                    List<Footballer> footballerList  = response.body().getFootballer();
